package agents.edsAgents.EDS_R_OLETSMCTS.voices.search.greedyTreeSearch;
...
public class TreeNode {
	...
	public TreeNode(StateObservation state, TreeNode parent) {
		...
		this.children = new TreeNode[GreedyTreeSearch.actions.length];
		this.value = 0;
	}

	public TreeNode SelectNode(){
		TreeNode current = this;
		while(!current.state.isGameOver() && current.depth < GreedyTreeSearch.MAX_DEPTH){
			...
			if(index < 0){
				...
				newState.advance(GreedyTreeSearch.actions[index]);
	...
	public double ExploreNode(){
		...
		while(depth < GreedyTreeSearch.MAX_DEPTH && !future.isGameOver())
		{
			future.advance(GreedyTreeSearch.actions[GetRandomActionIndex()]);
			depth+=1;
		}
	...
	private int GetRandomActionIndex(){
		return GreedyTreeSearch.random.nextInt(GreedyTreeSearch.actions.length);
	}
	
	private int GetUnexplored(){
		for(int i = 0; i<GreedyTreeSearch.actions.length; i++){
			if(children[i] == null){
				return i;
			}
		}
