package agents.edsAgents.EDS_AllActions.voices.Yolobot;
...
import agents.edsAgents.EDS_AllActions.voices.Yolobot.SubAgents.SubAgentStatus;
import agents.edsAgents.EDS_AllActions.voices.Yolobot.SubAgents.bfs.BFS;
import agents.edsAgents.EDS_AllActions.voices.Yolobot.Util.Heatmap;
import agents.edsAgents.EDS_AllActions.voices.Yolobot.Util.Heuristics.HeuristicList;
...
return null;
	...
	@Override
	public List<Opinion> performAnalysis(StateObservation stateObs, ElapsedCpuTimer elapsedTimer) {
...
//		System.out.println(YoloKnowledge.instance.toString());
			...
			if(currentYoloState.getGameTick() == 1){
				...
				if(ersterSchrittBugAufgetreten(currentYoloState)){
					...
					list.add(new Opinion(currentYoloState.getAvatarLastAction(), currentYoloState.getGameScore()));
					return list;
				}
			}
			//System.out.println(YoloKnowledge.instance.toString());
...
//					+ YoloState.advanceCounter);
			...
			List<Opinion> list = new ArrayList<>();
			list.add(new Opinion(chosenAction, 1.0f));
			return list;
		} catch (Throwable e) {
			e.printStackTrace();
			List<Opinion> list = new ArrayList<>();
			list.add(new Opinion(ACTIONS.ACTION_NIL, 0.0f));
			return list;
		}
	}
}
