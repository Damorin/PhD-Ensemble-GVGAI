package agents.competitorAgents.ICELab.BFS;
...
import agents.competitorAgents.ICELab.Agent;
							...
							if (so.getGameWinner() == WINNER.PLAYER_WINS) {
								if (!is_win) {
									...
									if (/*curr.rawvalue >= best_node.rawvalue
											||*/ so.getGameTick() > Agent.Max_GameTick * 4 / 5) {// 探索終わり
										GetPath(win_Node);
										getmove = true;
									} else {// まだ探索続けたほうがいいかもしれない
										System.out.println("continue search");
									}
								} else {
									if (so.getGameScore() > win_Node.rawvalue) {// より良い解が見つかった
										win_Node = new BFSNode(so, GameInfo.actions[act_count], curr, rand);
									}
